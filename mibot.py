...
# ========== PERSONAJE KAORI-CHAN ==========
NOMBRE_PERSONAJE = "Kaori-chan"
...
SALUDO = f"""🌸 *¡Hola! Soy {NOMBRE_PERSONAJE}!* ✨
...
MODELOS = {
    "1": {"id": "google/gemini-2.0-flash-exp:free", "nombre": "💎 Gemini 2.0 Flash", "desc": "Multimodal y súper rápido"},
    "2": {"id": "deepseek/deepseek-v4-flash:free", "nombre": "🌀 DeepSeek V4", "desc": "Muy inteligente y gratuito"},
...
async def preguntar_ai(prompt, chat_id, reintentos=2):
    ...
    # Prompt del sistema con la personalidad del personaje
    mensajes = [
...
async def enviar_respuesta(update, texto):
    """Envía texto largo sin cortar palabras, manteniendo formato Markdown"""
    if len(texto) <= 4000:
...
# ========== MENÚ PRINCIPAL (BOTONES BIEN VERGAS) ==========
def menu_principal():
    keyboard = [
...
async def reset(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(
        "🧹 *Historial reiniciado, nya~* ✨\n"
        "¡Ahora podemos empezar de nuevo! ¿Qué te gustaría hacer? 🌸",
        parse_mode=ParseMode.MARKDOWN,
        reply_markup=menu_principal()
    )
...
async def callback_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data == "conversar":
        await query.edit_message_text(
            "✏️ *¡Escríbeme lo que quieras, nya~!* 🐱\n\n"
